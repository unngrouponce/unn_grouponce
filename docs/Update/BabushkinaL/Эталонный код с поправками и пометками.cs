...
namespace Evaluator3D
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            // считываем файл .obj, считаем количество строк и количество символов в строке
            ...
            sr.Close();
            double[,] mas = new double[i, tmp.Length - 1];
            ...
            while ((s = sr.ReadLine()) != null)
            {
                ...
                {
                    mas[i,j-1] = Convert.ToDouble(tmp[j]);
                }
                i++;
            }
            ...
            //}
            if (args[0].ToUpper().EndsWith("DAT") &&
              args[1].ToUpper().EndsWith("OBJ"))
            {
                //generator(args[0], args[1]);
                evaluate_dat_dat(args[0], mas);
                return;
            }
            Console.Write("Evaluation is impossible. Undefined extension.\n");
            
        }
        //конец клиентского кода
        //static void evaluate_dat_dat(string fname1, string fname2)
        static void evaluate_dat_dat(string fname1, double[,] fname2)
        {
            //загружаем в массивы эталон и нашу модель
        ...
        {
            double[,] map = null;
            FileStream fs = new FileStream(fname, FileMode.Open);//Инициализирует новый экземпляр класса FileStream 
            //указанным путем и режимом создания FileMode.Open - для чтения и записи
            try
            {
                BinaryFormatter formatter = new BinaryFormatter();//Сериализует и десериализует объект
                //или полный граф связанных объектов в двоичном формате
                map = (double[,])formatter.Deserialize(fs);//Десериализует заданный поток в граф объектов
            }
            catch (SerializationException e)//ошибка сериализации
            {
        ...
        static void saveMap(double[,] map, string fname)//сохранение эталона
        {
            ...
            catch (SerializationException e)//если не получилось сохранить .dat в формат .bin
            {
        ...
        static bool evaluateMaps(double[,] map, double[,] any, out double err_max, out double err_sum, out int any_pnt)
        {
            ...
            err_sum = 0;
            any_pnt = 0;
            int width = map.GetLength(0);//получаем ширину матрицы эталона
            int height = map.GetLength(1);//получаем высоту матрицы эталона
            ...
            for (int i = 0; i < width; i++)
                for (int j = 0; j < height; j++)
                {
                    ...
                    any_pnt++;//считаем кол-во отрицательных высот в нашей модели
        ...
        {
            int charts = 10;//что за параметр?
            int[] chart = new int[charts];
            for (int i = 0; i < charts; i++) chart[i] = 0;

            err_max += 0.0000001;//зачем к максимальной ошибке прибавляем ещё 0.00000001??
            ...
            for (int i = 0; i < charts; i++)
            {
                Console.Write("errors {0:f}...{1:f}\t{2:f}%\n",
                    (i + 0) * err_max / charts,
                    (i + 1) * err_max / charts,
                    100.0 * chart[i] / (width * height));//что это за параметры?
            }
        }
        ...
        static bool calculateUniform(double[,] any, int level, int x, int y, int w, int h, int[] count)
        {
            if (level < count.Length)
            {
                ...
                h /= 2;
                bool present = false;
                //получаем количество точек в фокусе в областях
        ...
        static double evaluateUniform(int[] count)//считаем общее нормальное распределение (пока не процентное)
        {
            double result = 0;
            int total = 1;
